...
class OllamaClient:
    ...
    async def get_embedding(self, text: str) -> List[float]:
        ...
        logger.debug(f"get_embedding called with model={self.model}, endpoint={self.endpoint}, text[:30]={text[:30]!r}")
        if not text.strip():
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Embedding attempt {attempt+1} for text[:30]={text[:30]!r}")
                ...
                logger.debug(f"Embedding received for text[:30]={text[:30]!r}")
                return embedding
        ...
        logger.debug(f"All embedding attempts failed for text[:30]={text[:30]!r}")

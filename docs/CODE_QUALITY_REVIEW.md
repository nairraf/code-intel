...
|---|---------|----------|--------|--------|
| 1 | God module `server.py` | 🔴 | Large | Testability, maintainability |
| 2 | Duplicated sorting lambda ×5 | 🔴 | Small | DRY, bug risk |
| 3 | Global singleton instantiation | 🔴 | Medium | Testability, lifecycle |
| 4 | SQLite connection-per-call | 🟡 | Small | Performance |
| 5 | Pass 2 re-parses all files | 🟡 | Medium | Performance |
| 6 | `_recursive_chunk` spaghetti | 🟡 | Medium | Readability, bug risk  |
| 7 | Dual capture format handling | 🟡 | Small | DRY, readability |
| 8 | Inline imports in hot paths | 🟡 | Trivial | Code clarity |
| 9 | Bare/silent exception handling | 🟢 | Small | Debuggability |
| 10 | Config fallback bug | 🟢 | Trivial | Correctness |
| 11 | Repeated table-exists boilerplate | 🟢 | Small | DRY |
| 12 | Duplicate comment | 🟢 | Trivial | Cleanliness |

---

## Recommended Approach

1. **Wave 1 — Quick wins** (items 2, 8, 9, 10, 11, 12): Mechanical fixes, low risk, can be done in a single session.
2. **Wave 2 — `server.py` decomposition** (items 1, 3): Extract tools into modules, introduce DI. Medium risk — requires updating test imports.
3. **Wave 3 — Performance & complexity** (items 4, 5, 6, 7): Persistent connections, chunk caching, strategy pattern, capture normalization. Higher risk — needs thorough regression testing.

...
from src.tools.definition import _get_file_priority, find_definition_impl
from src.server import refresh_index
...
async def test_language_scoped_definition(tmp_path):
    """Verifies that definition jumps are prioritised by source language."""
    ...
    dart_dc = {
        ...
        "language": "dart", "symbol_name": "Settings", "type": "class",
    }
    py_dc = {
        "id": "py_settings",
        ...
        "language": "python", "symbol_name": "Settings", "type": "class",
    }

    def mock_find(root, name):
        return [dart_dc, py_dc] if name == "Settings" else []

    mock_ctx = MagicMock()
    mock_ctx.vector_store.find_chunks_by_symbol.side_effect = mock_find
    ...
    mock_ctx.parser._get_language.side_effect = mock_get_lang
